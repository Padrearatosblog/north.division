...
export function IntroSequence({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const entry = range(time, .82, INTRO_TIMELINE.firstWeld)
      const seatImpact = pulse(time, INTRO_TIMELINE.firstWeld, .14)
      ...
      root?.style.setProperty('--intro-entry', entry.toFixed(3))
      root?.style.setProperty('--intro-seat-impact', seatImpact.toFixed(3))
    ...
    <section ref={rootRef} className={`intro-experience ${running ? 'is-running' : 'is-gated'}`} aria-label="North Division cinematic experience">
      ...
      <div className="intro-scene">
        ...
        <div className="intro-scene__logo">
          <div className="intro-scene__blank"><BrandMark /></div>
          <BrandMark />
